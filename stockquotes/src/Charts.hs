...
plotChart title quotes fname = do
  ...
  where
    ...
    candlesLayout =
      layout_title .~ title $
        layout_plots
          .~ [ toPlot $ qline "Close" closings cyan,
               toPlot $ candle "Candle" candles red
             ]
          $ def
    volumesLayout =
      layout_plots .~ [plotBars $ bars "Volume" volumes gray] $ def
    candle label values color =
      plot_candle_line_style .~ lineStyle 1 gray $
        plot_candle_fill .~ True $
          plot_candle_rise_fill_style .~ fillStyle green $
